...
function searchRooms() {
  ...
  try {
    ...
    const result = rooms.filter(room => {
      const maxRoomCapacity = getMaxRoomTypeCapacity(room);
      const lowestPrice = getLowestRoomTypePrice(room);

      return (
        isRoomAvailableForBooking(room, booking.checkIn, booking.checkOut) &&
        maxRoomCapacity >= booking.guests &&
...
function validateBookingInputs(notice) {
  ...
  if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime())) {
    showNotice(notice, "error", "日期格式錯誤，請重新選擇。");
    return { valid: false };
  }
...
function getRoomDataIssues(room) {
  ...
  if (!Array.isArray(room.policies) || room.policies.length === 0) issues.push("住房政策");
